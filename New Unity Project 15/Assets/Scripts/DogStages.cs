...
public class DogStages : MonoBehaviour {
	...
	void Update () {
        float quantFurious = furious / maxfurious; //Ordem  health/max health(criar)
        dogStage.rectTransform.localScale = new Vector3(quantFurious, 1, 1);
        percent.text = (quantFurious * 100).ToString() + '%';
    }
    ...
    {
        return dogStage.rectTransform.localScale.x;
    }
}
